...
[Obsolete("", true)]
public class FSMChaseStateObsolete : FsmBaseStateObsolete<ChaseDeps>
{
   // private readonly ChaseDeps _depsNew;
    ...
    public FSMChaseStateObsolete(ChaseDeps deps, SharedFsmStateServices sharedDeps, IFsmStateEventsObsoleteO stateContext)
...
[Obsolete("", true)]
public sealed class FSMChaseStateNewObsolete : FsmBaseStateNew<IFsmStateService<ITargetContextObsolete>, ITargetContextObsolete>
{
...
*/
   ...
   //     _deps.PathResolver.ProcessDestinationCandidates(in req); // Connented out for now
    ...
    protected bool TargetIsMoving() => false;// Service.TargetIsMoving(_stateEvents);
   /* {
        *//*ITargetable target;
    ...
    public override void OnDestinationReached()
    {
        ...
        _initialDistance = 0f;//initialDistance;// = targetPos.SqrDistanceTo(ownerPos);
        RegisterDistanceCheck();
        // Start job to see if player/ target has moved far enough away
        // Add job callback
