...
public class FSMControllerObsolete : FSMControllerBaseNewObsolete
{
    ...
    protected override StateChangeResult ChangeStates(State newSate, Transform target = null, AlertStatus status = AlertStatus.None)
    {
        if(target != null) SetChaseTarget(target);

        return newSate switch
        {
            State.Patrol => ChangeStates(_patrol, status),
            State.Chase =>  ChangeStates(_chasing, status),
            State.Stationary => ChangeStates(_stationary, status),
            State.Death => ChangeStates(_deathState, status),
            _ => ChangeStates(_stationary, status)
        };
      
    }

    protected override StateChangeResult ChangeStates(EnemyStateObsolete state, AlertStatus status = AlertStatus.None)
    {
        if (state == null) return StateChangeResult.Failed;
        if (_currentState != null && _currentState == state) return StateChangeResult.AlreadyInState;
    ...
    protected override void ChangeState(EnemyStateObsolete state, AlertStatus status = AlertStatus.None)
    {
    ...
    public override void ResetFSM(EnemyStateObsolete contextState = null)
    {
        if (TargetInView)
        {
        ...
        switch (contextState)
        {
            case PatrolStateObsolete: // ResetFSM called when the player dies, or this agent is respawning

                break;
            case DeathStateObsolete: // ResetFSM called when this agent dies
