...
public sealed class DestinationProcessor : IDestinationResolver
{
    ...
    private Action<PathResult> PathValidationCallback;
    ...
    public DestinationProcessor(IPathService pathService, ICoroutineHost routineRunner)
    {
    ...
    private void OnPathValidationCallback(PathResult result)//bool pathFound/*in PathResult result*/)
    {
        // _isValid = pathFound;
        _lastResult = result;
        _pathChecked = true;
    ...
    public void CancelAll()
    {
        ...
        {
            _routineRunner.StopCoroutine(_runningRoutine);//_routineRunner.StopCoroutine(_runningRoutine);
            _runningRoutine = null;
        }
        while (_queue.Count > 0)
        {
            var (req, tcs) = _queue.Dequeue();
            DestinationResultInfo cancelled = new DestinationResultInfo(DestinationRequestReason.Cancelled, req.Path, DestinationResult.RequestCancelled, Vector3.zero, req.StateId);
    ...
    public Task<DestinationResultInfo> ProcessCandidates(in DestinationRequest request)
    {
        ...
        if (_runningRoutine == null)
            _runningRoutine = _routineRunner.StartCoroutine(PathFindRoutine(_queue));//_routineRunner.StartCoroutine(PathFindRoutineNewest(_queue));
    ...
    private IEnumerator PathFindRoutine(Queue<(DestinationRequest, TaskCompletionSource<DestinationResultInfo>)> q)
    {
