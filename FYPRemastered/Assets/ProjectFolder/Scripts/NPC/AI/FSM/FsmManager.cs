...
[Obsolete]
public class FsmManager : IFsmStateEventsObsoleteO, IFsmController
{
    // Injected Dependancies
    private IReadOnlyDictionary<StateId, IFsmStateObsolete> _states;
   ...
   // public Vector3? CurrentDestinationForward { get; private set; }
    ...
    private IFsmStateObsolete _currentState;
  ...
  //  public bool RotatingToTarget { get; private set; } = false;
    ...
    public FsmManager(FsmManagerServices deps, SharedFsmStateServices sharedDeps, IReadOnlyDictionary<StateId, IFsmStateObsolete> states, IPathNotifications pathNotifies, IAnimationRequestNotifications animNotifies = null/*, Notification fsmNotifications*/)
    {
        _deps = deps;
...
[Obsolete]
public class FsmManagerNew : IFsmStateEventsObsoleteO, IFsmController
{
    // Injected Dependancies
    private IReadOnlyDictionary<StateId, IFsmStateObsolete> _states;
   // private FsmManagerServices _deps;
  ...
  //  private bool OwnerIsNull() => SharedDepsIsNull() || _sharedDeps.OwnerTransform == null;
   ...
   // public bool HasReachedDestination() => !_hasValidDestination || (_deps?.Agent.isStopped ?? true);
    ...
    void IFsmStateEventsObsoleteO.Test()
    {
        throw new NotImplementedException();
    }

    bool IFsmStateEventsObsoleteO.TryGetTargetPosition(out Vector3? targetPos)
    {
        targetPos = null;
...
public class FsmManagerNewest : IFsmStateContext, IFsmController, ITargetProvider
{
  ...
  //  private readonly IFsmTargetQuery _targetQuery;
    ...
    public bool TryGetTarget(out ITargetable target)
    {
        target = null;
        if (_ownerTargetGetter is null) return false;
        return _ownerTargetGetter.Invoke(out target);
   ...
   // public bool HasReachedDestination() => !_hasValidDestination || (_deps?.Agent.isStopped ?? true);
    ...
    bool ITargetProvider.TryGetTargetPosition(out Vector3? targetPos)
    {
        targetPos = null;
        if (!TryGetTarget(out var target)) return false;
        targetPos = target.Position();
    ...
    public bool TryGetPath(out NavMeshPath path)
    {
        path = Path;
        return path is not null;
    }

    public bool TryGetCurrentPosition(out Vector3? currentPos)
    {
        currentPos = Position;
        return currentPos is not null;
    }
