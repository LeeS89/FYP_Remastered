...
[Obsolete("", true)]
public partial class NPCControllerOldObsolete : TargetableInit<ISceneAIServices, AgentEventManager>, INpcBody, /*IAgentData, */INpcBrainContext, INotificationListener, ICoroutineHost, ITickableRunner
{ // Remove IAgentData
   ...
   // public RotationOrder CurrentRotOrder { get; private set; } = RotationOrder.None;
    ...
    protected virtual void Update()
    {
        ...
        if (_testOverrideRot)
        {
            OnNotificationReceived(NpcNotification.PathNotifications.DestinationReached());
   ...
   // public RotationOrder CurrentRotOrder { get; private set; } = RotationOrder.None;
    ...
    public void OnNotificationReceived(in NpcNotification n)
    {
      ...
      //  if (n.Kind == NotificationKind.FOVUpdate) Debug.LogError("FOV Result: "+n.FOVResult.ToString());
       ...
       // _inbox.Enqueue(n);
        _brain?.Decide(context: this, in n);
    ...
    public void MapDestinationToZone(Vector3 destination)
    {
        ...
        ZoneId id;
        bool found = this.GetZoneIdNew(destination, out id);
        if (found)
        {
            if (id == _zoneId || id == ZoneId.Unknown) return;
            else
