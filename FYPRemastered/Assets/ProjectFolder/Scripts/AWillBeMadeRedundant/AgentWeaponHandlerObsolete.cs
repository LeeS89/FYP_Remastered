...
public class AgentWeaponHandlerObsolete : WeaponHandlerBaseObsolete
{
    ...
    public AgentWeaponHandlerObsolete(EnemyEventManager eventManager, PoolIdSO poolId, GameObject owner, Transform bulletSpawnPoint, Transform target, int clipCapacity) : base(owner, poolId)
    {
        ...
        _rangedWeapon = new AgentRangedWeaponObsolete(poolId, _bulletSpawnPoint, _target, _eventManager, _owner, _clipCapacity, PoolIsready);
    ...
    public override void EquipWeapon(WeaponTypeObsolete type)
    {
       ...
       // Debug.LogError("Weapon Equipped");
        ...
        if(_equippedWeapon is IRangedWeaponObsolete)
        {
            BeginFiringSequence();
        }
    }
    ...
    public override void UpdateEquippedWeapon()
    {
        ...
        if(_equippedWeapon is not IRangedWeaponObsolete || !IsTargetInView) 
        {
            if(_firingCoroutine != null)
            {
                CoroutineRunner.Instance.StopCoroutine(_firingCoroutine);
    ...
    {
        if(_equippedWeapon is IRangedWeaponObsolete rw)
        {
