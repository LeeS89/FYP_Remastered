...
[Obsolete("", true)]
public class DeathStateObsolete : EnemyStateObsolete
{
    private bool _deathAnimationComplete = false;
    private float _timeToDisable = 3;
    private float _countdown;

    public DeathStateObsolete(EnemyEventManager eventManager, GameObject owner) : base(eventManager, owner) { }
    

    public override void EnterState(AlertStatus alertStatus = AlertStatus.None)
    {
        _eventManager.ChangeAnimatorLayerWeight(AnimationLayer.Aim, 1, 0, 0.5f);
