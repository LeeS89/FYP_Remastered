    ...
    public unsafe partial struct AgentSpatialPartitioningSystem : ISystem
    {
       ...
       // [BurstCompile]
        ...
        public struct Singleton : IComponentData
        {
                    ...
                    while (handles.TryPop(out var handle))
                    {
                        ...
                        if (node.IsLeaf)
                        {
                            ...
                            if (!layers.Any(data->m_Agents[index].Layers))
                                continue;
                            if (!SegmentCylinderOverlap(from, to, data->m_Transforms[index].Position, data->m_Shapes[index].Height, data->m_Shapes[index].Radius))
                                continue;
            ...
            /// Query agents that intersect with the line.
            /// </summary>
            public QuerySphereEnumerator QuerySphereTight(float3 center, float radius, NavigationLayers layers = NavigationLayers.Everything)
            {
                var handles = new UnsafeStack<UnsafeBHVTree<AABB, int>.Handle>(16, Allocator.Temp);
                handles.Push(m_Tree->Root);
                var aabb = new AABB
                ...
                return new QuerySphereEnumerator
                {
                    data = (AgentSpatialPartitioningSystem.Singleton*) UnsafeUtility.AddressOf(ref this),
                    aabb = aabb,
                    layers = layers,
            ...
            /// Query agents that intersect with the line.
            /// </summary>
            public QueryCylinderEnumerator QueryCylinderTight(float3 center, float radius, float height, NavigationLayers layers = NavigationLayers.Everything)
            {
                var handles = new UnsafeStack<UnsafeBHVTree<AABB, int>.Handle>(16, Allocator.Temp);
                handles.Push(m_Tree->Root);
                var aabb = new AABB
                ...
                return new QueryCylinderEnumerator
                {
                    data = (AgentSpatialPartitioningSystem.Singleton*) UnsafeUtility.AddressOf(ref this),
                    aabb = aabb,
                    layers = layers,
                ...
                public bool MoveNext()
                {
                    ...
                    while (handles.TryPop(out var handle))
                    {
                        ...
                        if (node.IsLeaf)
                        {
                            ...
                            var otherPosition = data->m_Transforms[index].Position;
                            var otherShape = data->m_Shapes[index];
                            var agentAABB = new AABB
                            {
                                Min = otherPosition - new float3(otherShape.Radius, 0, otherShape.Radius),
                                Max = otherPosition + new float3(otherShape.Radius, otherShape.Height, otherShape.Radius),
                            };
                            if (!agentAABB.Overlap(aabb))
                                continue;
            ...
            public struct QuerySphereEnumerator :
                IEnumerator<(Entity entity, AgentBody body, AgentShape shape, LocalTransform transform)>,
                IDisposable
            {
                internal AgentSpatialPartitioningSystem.Singleton* data;
                ...
                public QuerySphereEnumerator GetEnumerator() => this;
                ...
                public bool MoveNext()
                {
                    ...
                    while (handles.TryPop(out var handle))
                    {
                        ...
                        if (node.IsLeaf)
                        {
                            ...
                            //     Max = otherPosition + new float3(otherShape.Radius, otherShape.Height, otherShape.Radius),
                            // };
                            if (!SphereCylinderOverlap(center, radius, otherPosition, otherShape.Radius, otherShape.Height))
                                continue;
            ...
            public struct QueryCylinderEnumerator :
                IEnumerator<(Entity entity, AgentBody body, AgentShape shape, LocalTransform transform)>,
                IDisposable
            {
                internal AgentSpatialPartitioningSystem.Singleton* data;
                ...
                public QueryCylinderEnumerator GetEnumerator() => this;
                ...
                public bool MoveNext()
                {
                    ...
                    while (handles.TryPop(out var handle))
                    {
                        ...
                        if (node.IsLeaf)
                        {
                            ...
                            var otherPosition = data->m_Transforms[index].Position;
                            var otherShape = data->m_Shapes[index];
                            if (!CylinderCylinderOverlap(center, radius, height, otherPosition, otherShape.Radius, otherShape.Height))
                                continue;
            ...
            // Cylinder is defined by two endpoints (p0, p1) and radius
            // Sphere is defined by center and radius
            static bool SphereCylinderOverlap(
                float3 sphereCenter,
                float sphereRadius,
                float3 cylP0,
                float3 cylP1,
                float cylRadius)
            {
                float3 axis = cylP1 - cylP0;
                float axisLenSq = lengthsq(axis);

                // Handle degenerate cylinder (treat as sphere)
                if (axisLenSq < 1e-6f)
                {
                    float r = sphereRadius + cylRadius;
                    return lengthsq(sphereCenter - cylP0) <= r * r;
                }

                float3 dir = axis * rsqrt(axisLenSq); // normalized axis (SIMD friendly)
                float height = sqrt(axisLenSq);

                // Project sphere center onto cylinder axis
                float t = dot(sphereCenter - cylP0, dir);
                t = clamp(t, 0f, height);

                float3 closestPoint = cylP0 + dir * t;

                float rSum = sphereRadius + cylRadius;
                return lengthsq(sphereCenter - closestPoint) <= rSum * rSum;
            }
            ...
            {
                // Project sphere center onto cylinder axis (Y)
                float3 d = sphereCenter - cylinderOrigin;

                // Clamp Y to cylinder height
                float y = clamp(d.y, 0f, cylinderHeight);

                // Closest point on cylinder axis
                float3 closest = float3(cylinderOrigin.x, cylinderOrigin.y + y, cylinderOrigin.z);

                // Radial direction from axis
                float3 radial = sphereCenter - closest;

                // Compute squared radial distance
                float distSq = radial.x * radial.x + radial.z * radial.z;

                // Clamp radial to cylinder radius (SIMD-friendly, no normalize)
                float maxRadial = cylinderRadius;
                float radialLen = sqrt(distSq);

                float scale = select(1f, maxRadial / radialLen, radialLen > maxRadial);
                float3 clampedRadial = float3(radial.x * scale, 0f, radial.z * scale);

                // Final closest point on cylinder surface
                float3 finalClosest = closest + clampedRadial;

                // Sphere test
                float3 diff = sphereCenter - finalClosest;
                float distFinalSq = dot(diff, diff);

                return distFinalSq <= sphereRadius * sphereRadius;
            }

            // Cylinder vs Cylinder (both vertical, Y axis)
            // Origins are bottom centers
            static bool CylinderCylinderOverlap(
                float3 aOrigin,
                float aRadius,
                float aHeight,
                float3 bOrigin,
            ...
            {
                // --- Vertical (Y) overlap ---
                float aMinY = aOrigin.y;
                float aMaxY = aOrigin.y + aHeight;

                float bMinY = bOrigin.y;
                float bMaxY = bOrigin.y + bHeight;

                bool yOverlap = (aMinY <= bMaxY) & (bMinY <= aMaxY);

                // --- Radial (XZ plane) overlap ---
                float2 aXZ = aOrigin.xz;
                float2 bXZ = bOrigin.xz;

                float2 d = aXZ - bXZ;
                float distSq = dot(d, d);

                float r = aRadius + bRadius;
                bool radialOverlap = distSq <= r * r;

                // Combine (bitwise AND for SIMD friendliness)
                return yOverlap & radialOverlap;
            }

    public static bool SegmentCylinderOverlap(float3 p0, float3 p1, float3 origin, float height, float radius)
    {
        float3 d = p1 - p0;
        float3 m = p0 - origin;
        ...
        // Shift to cylinder center for symmetric math
        m.y -= halfHeight;

        // Project onto XZ plane (radial test)
        float2 dXZ = d.xz;
        float2 mXZ = m.xz;

        float a = math.dot(dXZ, dXZ);
        float b = math.dot(mXZ, dXZ);
        float c = math.dot(mXZ, mXZ) - radius * radius;
        ...
        float sqrtDiscr = math.sqrt(discr);

        float invA = math.select(0f, 1f / a, a != 0f);
        ...
        float tEnter = math.max(t0, 0f);
        float tExit  = math.min(t1, 1f);

        if (tEnter > tExit)
            return false;

        // Check Y bounds for intersection interval
        float yEnter = m.y + d.y * tEnter;
        float yExit  = m.y + d.y * tExit;

        if (yEnter < -halfHeight && yExit < -halfHeight)
            return false;

        if (yEnter > halfHeight && yExit > halfHeight)
            return false;

        // Also check caps if segment is vertical or misses side
        if (math.abs(d.y) > 1e-6f)
        {
            // Bottom cap (y = -halfHeight)
            float tCap0 = (-halfHeight - m.y) / d.y;
            if (tCap0 >= 0f && tCap0 <= 1f)
            {
                float2 pCap = mXZ + dXZ * tCap0;
                if (math.lengthsq(pCap) <= radius * radius)
                    return true;
            }

            // Top cap (y = +halfHeight)
            float tCap1 = (halfHeight - m.y) / d.y;
            if (tCap1 >= 0f && tCap1 <= 1f)
            {
                float2 pCap = mXZ + dXZ * tCap1;
                if (math.lengthsq(pCap) <= radius * radius)
                    return true;
            }
        }
