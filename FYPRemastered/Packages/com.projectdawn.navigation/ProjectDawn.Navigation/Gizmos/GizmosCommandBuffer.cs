...
namespace ProjectDawn.Navigation
{
    public unsafe struct GizmosCommandBuffer : IDisposable
    {
        ...
        [Conditional("UNITY_EDITOR")]
        public void DrawLine(float3 from, float3 to, Color color, bool zTest = false)
        {
#if UNITY_EDITOR
        ...
        [Conditional("UNITY_EDITOR")]
        public void DrawAAConvexPolygon(Span<float3> vertices, Color color, bool zTest = false)
        {
#if UNITY_EDITOR
            fixed (float3* ptr = vertices)
            {
                ...
                Write(zTest);
                Write(vertices.Length);
                m_Commands.AddRange(ptr, sizeof(float3) * vertices.Length);
                WriteEnd();
            }
#endif
        }
