            ...
            public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in Unity.Burst.Intrinsics.v128 chunkEnabledMask)
            {
                ...
                unsafe
                {
                    ...
                    for (int i = 0; i < chunk.Count; i++)
                    {
                        ...
                        if (gg != null && gg.Linecast(movementStates[i].closestOnNavmesh, localTransforms[i].Position, out var hit))
                        {
                            if (paths[i].Grounded == Grounded.XZ)
                                localTransforms[i].Position.xz = ((float3)hit.point).xz;
                            else
                                localTransforms[i].Position = hit.point;
                        }

                        var rg = AstarPath.active.data.recastGraph;
                        if (rg != null && rg.Linecast(movementStates[i].closestOnNavmesh, localTransforms[i].Position, null, out var rhit))
                        {
                            if (paths[i].Grounded == Grounded.XZ)
                                localTransforms[i].Position.xz = ((float3)rhit.point).xz;
                            else
                                localTransforms[i].Position = rhit.point;
                        }
                    }
                }
