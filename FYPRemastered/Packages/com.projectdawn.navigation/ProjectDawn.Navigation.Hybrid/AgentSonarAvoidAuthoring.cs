...
namespace ProjectDawn.Navigation.Hybrid
{
    ...
    public class AgentAvoidAuthoring : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            if (UseWalls)
            {
                if (!gameObject.TryGetComponent<INavMeshWallProvider>(out var provider))
                    throw new System.InvalidOperationException("Property UseWalls can not be enabled without AgentNavMeshAuthoring component!");
                provider.AddComponents(world.EntityManager, m_Entity, Radius);
            }
        }
        ...
        void OnDestroy()
        {
            ...
            if (UseWalls)
            {
                if (!gameObject.TryGetComponent<INavMeshWallProvider>(out var provider))
                    throw new System.InvalidOperationException("Property UseWalls can not be enabled without AgentNavMeshAuthoring component!");
                provider.RemoveComponents(world.EntityManager, m_Entity);
            }
        }
